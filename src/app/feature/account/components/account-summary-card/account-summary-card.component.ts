...
export class AccountSummaryCardComponent {
  ...
  previousSummary = computed(() => this.summaryResource.value()?.previousMonth ?? { income: 0, expense: 0 });

  readonly incomeStats = computed(() => {
    const change = this.calculateChange(this.currentSummary().income, this.previousSummary().income);
    return { change, color: this.getBadgeColor(change) };
  });

  readonly expenseStats = computed(() => {
    const change = this.calculateChange(this.currentSummary().expense, this.previousSummary().expense);
    return { change, color: this.getBadgeColor(change) };
  });
